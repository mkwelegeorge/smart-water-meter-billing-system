...
class Login extends DBConnection {
	...
	public function login(){
		...
		$stmt = $this->conn->prepare("SELECT * from users where username = ? and password = ? and `type` !=4");
	...
	public function user_login(){
		extract($_POST);
		$stmt = $this->conn->prepare("SELECT * from individual_list where `email` = ? and password = ? and `status` != 4 ");
		$password = md5($password);
